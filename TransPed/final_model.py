...
parser.add_argument('--target_pep_file', type = str, help = 'the path of the .fasta file contains sequence')
...
with open(args.target_pep_file, 'r') as f:
    target_pep_file = f.readlines()
	
if len(peptide_file) != len(target_pep_file):
    log = Logger(errLogPath)
...
for pep, hla in zip(peptide_file, target_pep_file):
    if i % 2 == 0:
...
if args.output_mutation:
    ...
    for idx in range(predict_data.shape[0]):
        ...
        if mut_savepath:
            sanitized_hla = sanitize_filename(hla)
            sanitized_peptide = sanitize_filename(peptide)
            file_path = os.path.join(mut_savepath, '{}_mutation.csv'.format( sanitized_peptide))
            mut_data.to_csv(file_path, index=False)
            print('| {} → # Mutation peptides = {}'.format(peptide, mut_data.shape[0] - 1))
...
# python final_model.py --peptide_file "peptides.fasta" --target_pep_file "target.fasta" --threshold 0.5 --cut_length 10 --cut_peptide True --output_dir './results/' --output_attention True --output_heatmap True --output_mutation True