...
def pHLA_attns_draw_save(data, attn_data, hla = False, peptide = False, attn_savepath = False, fig_savepath = False):
    ...
    if attn_savepath:
        ...
        file_path = os.path.join(attn_savepath, '{}_{}_attention.csv'.format(sanitized_hla, sanitized_peptide))
    ...
    if fig_savepath:
        ...
        cmap = 'viridis'
        sns.heatmap(temp_pd.iloc[:-3].T, cmap=cmap, square=True, xticklabels=True, yticklabels=True, cbar=True)
        plt.ylabel('Peptide')
        plt.xlabel('Target peptide')
        plt.title('Target peptide | {} | {} | Probability = {}'.format(peptide, ['Negative', 'Positive'][y_pred == 1], y_prob))

        sanitized_hla = sanitize_filename(hla)
        sanitized_peptide = sanitize_filename(peptide)
        file_path = os.path.join(fig_savepath, 'target peptide_{}_{}_Prob{}.tif'.format(sanitized_peptide,
                                                                            ['Negative', 'Positive'][y_pred == 1],
                                                                            y_prob))
        plt.savefig(file_path, dpi=600, pil_kwargs={'compression': 'tiff_lzw'}, bbox_inches='tight',
